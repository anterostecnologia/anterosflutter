...
class AnterosAnimation extends StatefulWidget {
  ...
  @override
  _AnterosAnimationState createState() => _AnterosAnimationState();
}

class _AnterosAnimationState extends State<AnterosAnimation>
  ...
  @override
  void initState() {
    if (widget.type == AnterosAnimationType.rotateTransition) {
    ...
    } else if (widget.type == AnterosAnimationType.scaleTransition) {
      controller = widget.controller ??
          AnimationController(
              duration: widget.duration ?? const Duration(seconds: 2),
              vsync: this);
      ...
      controller?.forward();
    } else if (widget.type == AnterosAnimationType.slideTransition) {
  ...
  Widget getAnimatedTypeWidget() {
    switch (widget.type) {
      case AnterosAnimationType.align:
        return buildAnimatedAlignWidget();

      case AnterosAnimationType.container:
        return buildAnimatedContainerWidget();

      case AnterosAnimationType.size:
        return buildAnimatedSizeWidget();

      case AnterosAnimationType.rotateTransition:
        return buildRotationTransitionWidget();

      case AnterosAnimationType.scaleTransition:
        return buildScaleTransitionWidget();

      case AnterosAnimationType.slideTransition:
        return buildSlideTransitionWidget();

      case AnterosAnimationType.textStyle:
