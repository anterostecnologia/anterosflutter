...
///
/// GlassContainer paints border using [CustomPaint] with [AnterosRectBorderPainter]
/// or [AnterosCircleBorderPainter] depending on the box shape.
///
...
class AnterosGlassContainer extends StatelessWidget {
  ...
  AnterosGlassContainer({
    ...
    this.child,
        ...
        assert(color != null || gradient != null,
            'Tanto a cor quanto o gradiente não podem ser nulos\n'),
        assert(borderColor != null || borderGradient != null,
            'Ambos borderColor e borderGradient não podem ser nulos\n'),
        assert(shape != BoxShape.circle || borderRadius == null,
            'O [borderRadius] precisa ser nulo se a forma for [BoxShape.circle]\n'),
        assert(kIsWeb != true || borderColor != null,
            'borderColor não pode ser nulo quando executado na Web\n'),
        super(key: key);

  /// Creates a widget that extends [AnterosGlassContainer] to implement a clear glass
  /// effect.
  ...
  AnterosGlassContainer.clearGlass({
    Key? key,
  ...
  /// Creates a widget that extends [AnterosGlassContainer] to implement a frosted glass
  /// effect.
  /// Its a default implementation of the effect with editable decorations
  ///
  /// * If `color` and `gradient` are null, default value is assigned to gradient.
  ...
  ///
  /// See [Constants](https://pub.dev/documentation/glass_kit/latest/glass_kit/glass_kit-library.html#constants)
  AnterosGlassContainer.frostedGlass({
    Key? key,
    required double height,
    required double width,
    AlignmentGeometry? alignment,
  ...
  Widget build(BuildContext context) {
    ...
    if (!_colorOnlyBorder && !kIsWeb) {
      ...
      if (_isCircle) {
        ...
        current = CustomPaint(
          painter: AnterosCircleBorderPainter(
            radius: height / 2,
            gradient: borderGradient!,
        ...
        assert(borderRadius != null);
        current = CustomPaint(
          painter: AnterosRectBorderPainter(
...
class _FrostedWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Opacity(
      ...
      child: Image(
        image: ResizeImage(
          AssetImage(kNoiseImage, package: 'anterosflutter'),
          height: height.toInt(),
          width: width.toInt(),
        ),
...
/// The default values used by the [AnterosGlassContainer]
...
class AnterosRectBorderPainter extends CustomPainter {
  AnterosRectBorderPainter({
...
///
/// Used by GlassContainer to paint the gradient borders
class AnterosCircleBorderPainter extends CustomPainter {
  AnterosCircleBorderPainter({
    required this.radius,
    required this.gradient,
    required this.strokeWidth,
  }) : _paint = Paint();
