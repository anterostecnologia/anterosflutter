...
class AnterosSideEventArranger<T> extends AnterosEventArranger<T> {
  /// This class will provide method that will arrange
  /// all the events side by side.
  const AnterosSideEventArranger();

  @override
  List<OrganizedCalendarEventData<T>> arrange({
    required List<AnterosCalendarEventData<T>> events,
    ...
    final table = List.generate(
      ...
      (index) => List.generate(
        durations.length,
        (index) =>
            null as AnterosCalendarEventData<T>?, // ignore: unnecessary_cast
      ),
    );
    ...
    for (var i = 0; i < rowCounter; i++) {
      AnterosCalendarEventData<T>? event;
  ...
  void _insertIntoTable(List<List<AnterosCalendarEventData<T>?>> table,
      List<int> durations, int row, AnterosCalendarEventData<T> event) {
    var i = 0;
  ...
  /// in ascending order.
  List<int> _getEventsDuration(List<AnterosCalendarEventData<T>> events) {
    final durations = <int>[];
