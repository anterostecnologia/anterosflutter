...
///
/// To access the storage mechanism an instance of the [AnterosCacheProvider] implementation
/// is required.
///
...
///
///
class AnterosSettings {
  /// Private constructor
  AnterosSettings._internal();

  /// Private instance of this class to keep it singleton
  static final AnterosSettings _instance = AnterosSettings._internal();

  /// Public factory method to provide the
  factory AnterosSettings() {
    assert(
        _cacheProvider != null,
  ...
  /// Private instance of [AnterosCacheProvider] which will allow access to the
  /// underlying cache mechanism, which can be any of [SharedPreference],[Hive]
  /// or any other cache provider of choice
  static AnterosCacheProvider? _cacheProvider;
  ...
  static Future<void> init({AnterosCacheProvider? cacheProvider}) async {
...
class ValueChangeNotifier<T> extends ValueNotifier<T> {
  ...
  set value(T newValue) {
    AnterosSettings.setValue<T>(key, newValue);
...
class _ValueChangeObserverState<T> extends State<ValueChangeObserver<T>> {
  ...
  void initState() {
    ...
    final containsKey = AnterosSettings.containsKey(cacheKey) ?? false;
    if (!containsKey) {
      AnterosSettings.setValue<T>(cacheKey, defaultValue);
    }

    // get cache value from the [cacheProvider]
    value = AnterosSettings.getValue<T>(cacheKey, defaultValue);
