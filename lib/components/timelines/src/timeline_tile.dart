...
enum AnterosTimelineNodeAlign {
  /// Align [AnterosTimelineTile.node] to start side.
  start,

  /// Align [AnterosTimelineTile.node] to end side.
  end,

  /// Align according to the [AnterosTimelineTile.nodePosition].
  basic,
}
...
class AnterosTimelineTile extends StatelessWidget {
  const AnterosTimelineTile({
    Key? key,
    this.direction,
    required this.node,
    this.nodeAlign = AnterosTimelineNodeAlign.basic,
    this.nodePosition,
    this.contents,
    ...
    this.crossAxisExtent,
          ...
          nodeAlign == AnterosTimelineNodeAlign.basic ||
              (nodeAlign != AnterosTimelineNodeAlign.basic &&
                  nodePosition == null),
  ...
  /// This can be prevented by set [AnterosIndicatorThemeData.size] to an appropriate
  /// size.
  ///
  /// If [nodeAlign] is not [AnterosTimelineNodeAlign.basic], then [nodePosition] is
  /// ignored.
  final AnterosTimelineNodeAlign nodeAlign;
  ...
  double _getEffectiveNodePosition(BuildContext context) {
    if (nodeAlign == AnterosTimelineNodeAlign.start) return 0.0;
    if (nodeAlign == AnterosTimelineNodeAlign.end) return 1.0;
    var nodePosition = this.nodePosition;
    nodePosition ??= (node is AnterosTimelineTileNode)
        ? (node as AnterosTimelineTileNode).getEffectivePosition(context)
        : AnterosTimelineTheme.of(context).nodePosition;
    return nodePosition;
  }
  ...
  Widget build(BuildContext context) {
    ...
    var minNodeExtent =
        AnterosTimelineTheme.of(context).indicatorTheme.size ?? 0.0;
    var items = [
      if (nodeFlex > 0)
        Expanded(
          flex: nodeFlex.toInt(),
