...
@immutable
class AnterosGridArea {
  const AnterosGridArea({
    this.name,
  ...
  const AnterosGridArea.withSpans({
    this.name,
    required this.columnStart,
  ...
  bool operator ==(dynamic other) {
    ...
    return other is AnterosGridArea &&
        other.name == name &&
...
class NamedGridAreas {
  NamedGridAreas({
    ...
    required Map<String, AnterosGridArea> areas,
  }) : _areas = areas;

  final int columnCount;
  final int rowCount;
  final Map<String, AnterosGridArea> _areas;

  /// The number of named areas
  int get length => _areas.length;

  /// The [AnterosGridArea] named [areaName], or `null` if it does not exist
  AnterosGridArea? operator [](String? areaName) => _areas[areaName!];
}
...
/// Determines the region of a [AnterosGridArea] by adding individual (column, row)
/// pairs.
