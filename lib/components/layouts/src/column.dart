...
class AnterosColumn extends MultiChildRenderObjectWidget {
  //
  ...
  static List<Widget> _childrenPlusSeparator(
      List<Widget?> children, Widget? separator) {
    List<Widget> list =
        List.of(children.where((child) => child != null).cast());
    if (separator != null && children.isNotEmpty) list.add(separator);
    return list;
  }

  @override
  _RenderColumnSuperBox createRenderObject(BuildContext context) =>
  ...
  void updateRenderObject(
      BuildContext context, _RenderColumnSuperBox renderObject) {
    renderObject
      ..outerDistance = outerDistance
      ..innerDistance = innerDistance
      ..invert = invert
...
class _RenderColumnSuperBox extends RenderBox
  ...
  void performLayout() {
    ...
    if (_children.isEmpty) {
      ...
      maxChildWidth =
          max(min(maxChildWidth, constraints.maxWidth), constraints.minWidth);
      ...
      size = constraints
          .constrain(Size(maxChildWidth, dy - innerDistance + outerDistance));
    }
  }
  ...
  double computeMinIntrinsicHeight(double width) {
    ...
    if (_children.isNotEmpty && (_children.length != removed))
      dy += ((_children.length - removed - 1) * innerDistance) +
          (outerDistance * 2);
