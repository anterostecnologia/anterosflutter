...
enum AnterosWrapType {
  /// Will fit all widgets it can in a line, and then move to the next one.
  fit,

  /// The number of lines will be similar as in [AnterosWrapType.fit],
...
class AnterosWrap extends MultiChildRenderObjectWidget {
  ...
  AnterosWrap({
    Key? key,
    this.wrapType = AnterosWrapType.balanced,
    this.wrapFit = AnterosWrapFit.min,
    this.spacing = 0.0,
    this.lineSpacing = 0.0,
    this.alignment = AnterosWrapAlignment.left,
    List<Widget> children = const [],
  }) : super(key: key, children: children);

  final AnterosWrapAlignment alignment;
  ...
  final AnterosWrapType wrapType;

  final AnterosWrapFit wrapFit;
...
class _RenderWrapSuper extends RenderBox
  ...
  _RenderWrapSuper({
    ...
    AnterosWrapAlignment alignment = AnterosWrapAlignment.left,
    AnterosWrapType wrapType = AnterosWrapType.balanced,
    AnterosWrapFit wrapFit = AnterosWrapFit.min,
  })  : _spacing = spacing,
  ...
  AnterosWrapAlignment get alignment => _alignment;
  AnterosWrapAlignment _alignment;

  set alignment(AnterosWrapAlignment value) {
    if (_alignment == value) return;
    _alignment = value;
    markNeedsLayout();
  }

  AnterosWrapType get wrapType => _wrapType;
  AnterosWrapType _wrapType;

  set wrapType(AnterosWrapType value) {
    if (_wrapType == value) return;
    _wrapType = value;
    markNeedsLayout();
  }

  AnterosWrapFit get wrapFit => _wrapFit;
  AnterosWrapFit _wrapFit;

  set wrapFit(AnterosWrapFit value) {
  ...
  void performLayoutMin() {
    ...
    if (wrapType == AnterosWrapType.balanced) {
      List<List<num>> result = AnterosMinimumRaggedness.divide(
    ...
    //
    // Will fit all widgets it can in a line, and then move to the next one.
    else if (wrapType == AnterosWrapType.fit) {
      double x = 0;
      _Line line = _Line();
      lines.add(line);
    ...
    for (_Line line in lines) {
      double x;
      if (alignment == AnterosWrapAlignment.left)
        x = 0;
      else if (alignment == AnterosWrapAlignment.right)
        x = availableWidth - line.width;
      else if (alignment == AnterosWrapAlignment.center)
