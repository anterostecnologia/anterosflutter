...
import 'package:anterosflutter/components/layouts/anteros_layout_widgets.dart';
import 'package:flutter/material.dart';
import 'package:flutter/rendering.dart';

class AnterosRow extends MultiChildRenderObjectWidget {
  //
  ...
  static List<Widget> _childrenPlusSeparator(
    ...
    if (fitHorizontally)
      iterable = iterable.map((child) {
        return (child is AnterosRowSpacer) //
            ? child
            : AnterosFitHorizontally(shrinkLimit: shrinkLimit, child: child);
      });
  ...
  @override
  _RenderAnterosRowBox createRenderObject(BuildContext context) =>
      _RenderAnterosRowBox(
  ...
  void updateRenderObject(
      BuildContext context, _RenderAnterosRowBox renderObject) {
    renderObject
      ..outerDistance = outerDistance
      ..innerDistance = innerDistance
      ..invert = invert
...
class _RenderAnterosRowBox extends RenderBox
    with
  ...
  void _performLayout() {
    ...
    for (RenderBox child in _children!) {
      if (child is AnterosRenderRowSpacer) numberOfSpacers++;
      var maxIntrinsicWidth =
    ...
    for (int i = 0; i < _children!.length; i++) {
      ...
      if (child is AnterosRenderRowSpacer) dx += spacerWidth;
    }
...
class AnterosRowSpacer extends MultiChildRenderObjectWidget {
  @override
  RenderObject createRenderObject(BuildContext context) {
    return AnterosRenderRowSpacer();
  }
}

class AnterosRenderRowSpacer extends RenderBox
    with
