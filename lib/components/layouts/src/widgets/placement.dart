...
/// A widget that controls where a child of a [AnterosLayoutGrid] is placed. If a grid
/// item is not wrapped by a [AnterosGridPlacement], it will be placed in the first
/// available space, spanning one row and one column.
class AnterosGridPlacement extends ParentDataWidget<AnterosGridParentData> {
  const AnterosGridPlacement({
    Key? key,
  ...
  void applyParentData(RenderObject renderObject) {
    assert(renderObject.parentData is AnterosGridParentData);
    final parentData = renderObject.parentData as AnterosGridParentData;
    ...
    if (needsLayout) {
      ...
      if (targetParent is AnterosRenderLayoutGrid)
        targetParent.markNeedsPlacement();
  ...
  @override
  Type get debugTypicalAncestorWidgetClass => AnterosLayoutGrid;
}

/// Grid placement based on the name of an area provided to the grid via
/// [AnterosLayoutGrid.areas].
///
/// If [areaName] does not exist in the grid's [AnterosLayoutGrid.areas], the
/// child of this widget is not shown.
class NamedAreaGridPlacement extends ParentDataWidget<AnterosGridParentData> {
...
extension AnterosGridPlacementExtensions on Widget {
  ...
  AnterosGridPlacement withGridPlacement({
    Key? key,
    int? columnStart,
    int columnSpan = 1,
    int? rowStart,
