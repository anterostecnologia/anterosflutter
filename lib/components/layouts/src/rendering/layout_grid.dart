...
/// Parent data for use with [AnterosRenderLayoutGrid].
class AnterosGridParentData extends ContainerBoxParentData<RenderBox> {
  AnterosGridParentData({
    this.columnStart,
    this.columnSpan = 1,
    this.rowStart,
    this.rowSpan = 1,
  ...
  AnterosGridArea get area {
    assert(isDefinitelyPlaced);
    return AnterosGridArea(
      name: areaName,
  ...
  set area(AnterosGridArea? value) {
...
class AnterosRenderLayoutGrid extends RenderBox
    with
        ContainerRenderObjectMixin<RenderBox, AnterosGridParentData>,
        RenderBoxContainerDefaultsMixin<RenderBox, AnterosGridParentData>,
        DebugOverflowIndicatorMixin {
  /// Creates a layout grid render object.
  AnterosRenderLayoutGrid({
    ...
    double rowGap = 0,
    String? areasSpec,
    required List<AnterosTrackSize> columnSizes,
    required List<AnterosTrackSize> rowSizes,
  ...
  late AnterosPlacementGrid _placementGrid;
  ...
  List<AnterosTrackSize> get columnSizes => _columnSizes;
  List<AnterosTrackSize> _columnSizes;
  set columnSizes(List<AnterosTrackSize> value) {
  ...
  List<AnterosTrackSize> get rowSizes => _rowSizes;
  List<AnterosTrackSize> _rowSizes;
  set rowSizes(List<AnterosTrackSize> value) {
  ...
  void setupParentData(RenderBox child) {
    if (child.parentData is! AnterosGridParentData) {
      child.parentData = AnterosGridParentData();
    }
  }
  ...
  List<RenderBox> getChildrenInTrack(
      AnterosTrackType trackType, int trackIndex) {
  ...
  void performLayout() {
    ...
    while (child != null) {
      final parentData = child.parentData as AnterosGridParentData;
  ...
  GridSizingInfo computeGridSize(
    ...
    _performTrackSizing(
      AnterosTrackType.row,
      gridSizing,
      constraints: childConstraints,
    );

    // Stretch intrinsics
    _stretchIntrinsicTracks(AnterosTrackType.column, gridSizing,
        constraints: childConstraints);
    _stretchIntrinsicTracks(AnterosTrackType.row, gridSizing,
        constraints: childConstraints);
  ...
  List<GridTrack> _performTrackSizing(
    AnterosTrackType typeBeingSized,
  ...
  void _resolveIntrinsicTrackSizes(
    ...
    if (intrinsicTracks.isNotEmpty && debugPrintGridLayout) {
      debugPrint('Resolving intrinsic ${describeEnum(type)} '
          '${type == AnterosTrackType.column ? 'widths' : 'heights'} '
  ...
  void visitChildrenForSemantics(visitor) {
    var child = firstChild;
    while (child != null) {
      final AnterosGridParentData childParentData =
          child.parentData as AnterosGridParentData;
    ...
    assert(() {
      ...
      for (int c = 0; c < _columnSizes.length; c++) {
        for (int r = 0; r < _rowSizes.length; r++) {
          final cellRect = lastGridSizing.rectForArea(AnterosGridArea(
            columnStart: c,
            columnEnd: c + 1,
            rowStart: r,
            rowEnd: r + 1,
...
MinMax<double> constraintBoundsForType(
    BoxConstraints? constraints, AnterosTrackType type) {
  return type == AnterosTrackType.column
...
class GridTrack {
  ...
  final int index;
  final AnterosTrackSize sizeFunction;
...
UnmodifiableListView<GridTrack> _sizesToTracks(
        Iterable<AnterosTrackSize> sizes) =>
...
class GridSizingInfo {
  ...
  GridSizingInfo.fromTrackSizeFunctions({
    required List<AnterosTrackSize> columnSizeFunctions,
    required List<AnterosTrackSize> rowSizeFunctions,
    required TextDirection textDirection,
    double columnGap = 0,
    double rowGap = 0,
  }) : this(
  ...
  Offset offsetForArea(AnterosGridArea area) {
    return Offset(
  ...
  Size sizeForArea(AnterosGridArea area) {
    return Size(
      sizeForAreaOnAxis(area, Axis.horizontal),
  ...
  Rect rectForArea(AnterosGridArea area) {
    return offsetForArea(area) & sizeForArea(area);
  }

  void markTrackTypeSized(AnterosTrackType type) {
    if (type == AnterosTrackType.column) {
  ...
  List<double> baseSizesForType(AnterosTrackType type) =>
      tracksForType(type).map((t) => t.baseSize).toList();

  double totalBaseSizeOfTracksForType(AnterosTrackType type) =>
  ...
  double unitGapForType(AnterosTrackType type) =>
      unitGapAlongAxis(measurementAxisForTrackType(type));

  double totalGapForType(AnterosTrackType type) =>
  ...
  List<GridTrack> tracksForType(AnterosTrackType type) =>
      type == AnterosTrackType.column ? columnTracks : rowTracks;

  List<GridTrack> tracksAlongAxis(Axis sizingAxis) =>
      sizingAxis == Axis.horizontal ? columnTracks : rowTracks;

  double sizeForAreaOnAxis(AnterosGridArea area, Axis axis) {
  ...
  void invalidateTrackStartsForType(AnterosTrackType type) {
    if (type == AnterosTrackType.column) {
