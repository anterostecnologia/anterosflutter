...
///
class AnterosFitHorizontally extends SingleChildRenderObjectWidget {
  const AnterosFitHorizontally({
    Key? key,
    Widget? child,
    double? shrinkLimit,
    ...
    this.alignment = Alignment.center,
            ...
            shrinkLimit == null || (shrinkLimit >= 0.0 && shrinkLimit <= 1.0),
  ...
  void updateRenderObject(
      BuildContext context, RenderFitHorizontally renderObject) {
    renderObject
      ..shrinkLimit = shrinkLimit
      ..fitsHeight = fitsHeight
      ..alignment = alignment
...
class RenderFitHorizontally extends RenderProxyBox {
  ...
  RenderFitHorizontally({
    double shrinkLimit = AnterosFitHorizontally.defaultShrinkLimit,
    ...
    if (child != null) {
      ...
      if (_fitsHeight) {
        ...
        if (shrinkLimit == 1.0) {
          ...
          child!.layout(
              BoxConstraints(maxWidth: width, maxHeight: intrinsicHeight),
              parentUsesSize: true);
        }
        // But it's slower if there's shrink, since we also need intrinsicWidth.
          ...
          if (shrink! > shrinkLimit)
            ...
            child!.layout(
                BoxConstraints(
                    maxWidth:
                        (constraints.maxHeight == 0.0 || shrinkLimit == 0.0)
                            ? 0.0
                            : (intrinsicHeight *
      ...
      // Should NOT scaleVertically (_fitsHeight = false).
        ...
        // But it's slower if there's shrink, since we also need intrinsicWidth.
          ...
          shrink = (intrinsicWidth == 0.0)
              ? 1.0
              : (constraints.maxWidth / intrinsicWidth);
  ...
  static FittedSizes _applyBoxFit(
    ...
    // Should NOT scaleVertically.
      ...
      if (shrinkLimit == 1.0) {
        ...
        sourceSize = inputSize;
        destinationSize = Size(
            inputSize.width * max(shrinkLimit, min(shrink!, 1.0)),
            inputSize.height);
      }
    }
  ...
  void _updatePaintData() {
    ...
    if (child == null) {
      ...
      final FittedSizes sizes =
          _applyBoxFit(childSize, size, shrink, shrinkLimit, _fitsHeight);
      final double scaleX = (sizes.source.width == 0.0)
          ? 0.0
          : sizes.destination.width / sizes.source.width;
      final double scaleY = (sizes.source.height == 0.0)
          ? 0.0
