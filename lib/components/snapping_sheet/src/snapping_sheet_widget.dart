...
import 'package:anterosflutter/components/snapping_sheet/src/above_sheet_size_calculator.dart';
import 'package:anterosflutter/components/snapping_sheet/src/below_sheet_size_calculator.dart';
import 'package:anterosflutter/components/snapping_sheet/src/on_drag_wrapper.dart';
import 'package:anterosflutter/components/snapping_sheet/src/sheet_content_wrapper.dart';
import 'package:anterosflutter/components/snapping_sheet/src/sheet_position_data.dart';
import 'package:anterosflutter/components/snapping_sheet/src/snapping_calculator.dart';
import 'package:flutter/widgets.dart';

class AnterosSnappingSheet extends StatefulWidget {
  /// The content for the above part of the sheet.
  ///
  /// Needs to be of type
  /// [AnterosSnappingSheetContent] where a widget can be passed in.
  final AnterosSnappingSheetContent? sheetAbove;

  /// The content for the below part of the sheet.
  ///
  /// Needs to be of type
  /// [AnterosSnappingSheetContent] where a widget can be passed in.
  final AnterosSnappingSheetContent? sheetBelow;
  ...
  ///
  /// ```dart
  /// AnterosSnappingPosition.factor(positionFactor: 0.25)
  /// AnterosSnappingPosition.pixels(positionPixels: 420)
  /// ```
  ///
  ...
  /// ),
  /// ```
  final List<AnterosSnappingPosition> snappingPositions;
  ...
  final AnterosSnappingPosition? initialSnappingPosition;

  /// The controller for executing commands and reading current status of the
  /// [AnterosSnappingSheet]
  final AnterosSnappingSheetController? controller;

  /// Callback for when the sheet moves.
  ///
  /// Is called every time the sheet moves, both when snapping and moving
  /// the sheet manually.
  final Function(AnterosSheetPositionData positionData)? onSheetMoved;

  /// Callback for when a snapping animation is completed.
  final Function(
    AnterosSheetPositionData positionData,
  ...
  AnterosSnappingSheet({
    ...
    this.snappingPositions = const [
      ...
      AnterosSnappingPosition.factor(positionFactor: 0.5),
      AnterosSnappingPosition.factor(
        positionFactor: 1.0,
  ...
  AnterosSnappingSheet.horizontal({
    Key? key,
    AnterosSnappingSheetContent? sheetRight,
    AnterosSnappingSheetContent? sheetLeft,
    this.grabbing = const SizedBox(),
  ...
  @override
  _AnterosSnappingSheetState createState() => _AnterosSnappingSheetState();
}

class _AnterosSnappingSheetState extends State<AnterosSnappingSheet>
    with TickerProviderStateMixin {
  double _currentPositionPrivate = 0;
  BoxConstraints? _latestConstraints;
  late AnterosSnappingPosition _lastSnappingPosition;
  ...
  @override
  void didUpdateWidget(covariant AnterosSnappingSheet oldWidget) {
  ...
  void _setSheetLocationData() {
    if (widget.sheetAbove != null)
      widget.sheetAbove!.location = AnterosSheetLocation.above;
    if (widget.sheetBelow != null)
      widget.sheetBelow!.location = AnterosSheetLocation.below;
  }
  ...
  AnterosSnappingPosition get _initSnappingPosition {
    return widget.initialSnappingPosition ?? widget.snappingPositions.first;
  }

  AnterosSheetPositionData _createPositionData() {
    return AnterosSheetPositionData(
      _currentPosition,
  ...
  TickerFuture _snapToPosition(AnterosSnappingPosition snappingPosition) {
  ...
  TickerFuture _animateToPosition(AnterosSnappingPosition snappingPosition) {
  ...
  AnterosSnappingCalculator _getSnappingCalculator() {
    return AnterosSnappingCalculator(
  ...
  Widget buildGrabbingWidget() {
    ...
    final dragWrapper = AnterosOnDragWrapper(
      axis: widget.axis,
      dragEnd: _dragEnd,
        ...
        return Container(
          ...
          child: Stack(
            children: [
              ...
              AnterosSheetContentWrapper(
                axis: widget.axis,
                dragEnd: _dragEnd,
                dragUpdate: _dragSheet,
                currentPosition: _currentPosition,
                snappingCalculator: _getSnappingCalculator(),
                sizeCalculator: AnterosAboveSheetSizeCalculator(
                  axis: widget.axis,
              ...
              AnterosSheetContentWrapper(
                ...
                currentPosition: _currentPosition,
                snappingCalculator: _getSnappingCalculator(),
                sizeCalculator: AnterosBelowSheetSizeCalculator(
                  axis: widget.axis,
...
class AnterosSnappingSheetController {
  _AnterosSnappingSheetState? _state;
  ...
  void _attachState(_AnterosSnappingSheetState state) {
    _state = state;
  }
  ...
  TickerFuture snapToPosition(AnterosSnappingPosition snappingPosition) {
    _checkAttachment();
  ...
  AnterosSnappingPosition get currentSnappingPosition {
    _checkAttachment();
